...
router.put(
  ...
  asyncRouteHandler(async (req: RequestUser, res) => {
    ...
    partys[0] = await Party(domain, instance).findByIdAndUpdate({ _id: partys[0]._id }, partys[0], { new: true }).exec();
    partys[1] = await Party(domain, instance).findByIdAndUpdate({ _id: partys[1]._id }, partys[1], { new: true }).exec();
    let x = await Party(domain, instance).find({ name: partys[1].name });
    let y = await Party(domain, instance).find({ name: partys[0].name });

    if (x.length > 1) {
      await Party(domain, instance).findByIdAndDelete({ _id: x[1]._id }).exec();
    }

    if (y.length > 1) {
      await Party(domain, instance).findByIdAndDelete({ _id: y[1]._id }).exec();
    }
