...
router.put(
  ...
  asyncRouteHandler(async (req: RequestUser, res) => {
    ...
    items[0] = await Item(domain, instance).findByIdAndUpdate({ _id: items[0]._id }, items[0], { new: true }).exec();
    items[1] = await Item(domain, instance).findByIdAndUpdate({ _id: items[1]._id }, items[1], { new: true }).exec();
    let x = await Item(domain, instance).find({ name: items[1].name });
    let y = await Item(domain, instance).find({ name: items[0].name });

    if (x.length > 1) {
      await Item(domain, instance).findByIdAndDelete({ _id: x[1]._id }).exec();
    }

    if (y.length > 1) {
      await Item(domain, instance).findByIdAndDelete({ _id: y[1]._id }).exec();
    }
