...
export const getPartyPropfromGtm = `<sql2xml><Query><RootName>GTM</RootName><Statement>\nselect(select listagg(REMARK_QUAL_GID, ',') within group (order by REMARK_QUAL_GID) from (select REMARK_QUAL_GID, sum(length(REMARK_QUAL_GID) + 2) over (order by REMARK_QUAL_GID) running_length from REMARK_QUAL) where running_length &lt; 4600 and REMARK_QUAL_GID NOT LIKE 'HAZMAT%'and REMARK_QUAL_GID NOT LIKE 'ITEM_%' and REMARK_QUAL_GID NOT LIKE 'PARTY_%' and REMARK_QUAL_GID NOT LIKE 'BIIB.NET_WEIGHT%' and REMARK_QUAL_GID NOT LIKE 'BIIB.UOM%' and REMARK_QUAL_GID NOT LIKE 'BIIB.GROSS_WEIGHT%' and REMARK_QUAL_GID NOT LIKE 'BIIB.TOTAL_PACKAGING_UNIT_COUNT%') REMARKS, (SELECT listagg(GTM_PARTY_REFNUM_QUAL_GID,',') within group (order by GTM_PARTY_REFNUM_QUAL_GID) FROM GTM_PARTY_REFNUM_QUAL) CONTACT_REFNUMS from dual\n</Statement></Query></sql2xml>`;
