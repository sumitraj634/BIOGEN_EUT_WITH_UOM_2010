...
function invalidLineUOM(csvHeader: any[], csvHeaderCounter: number, validationData: any, cellValue: any) {
  return (
    csvHeader[csvHeaderCounter] === 'Line Quantity UOM' &&
    !isEmpty(validationData.transaction) &&
    !isEmpty(validationData.transaction.quantity) &&
    !validationData.transaction.quantity[cellValue]
  );
}
//for Qty Qualifier Net Weight UOM
...
//for Qty Qualifier Gross Weight UOM
...
//for Qty Qualifier Total Packaging UOM
function invalidTotalPackagingUnitCountUOM(
  csvHeader: any[],
  csvHeaderCounter: number,
  validationData: any,
  cellValue: any
  ...
  return (
    csvHeader[csvHeaderCounter] === 'Line Uom Total Packaging Unit Count' &&
    !isEmpty(validationData.transaction) &&
    !isEmpty(validationData.transaction.quantity) &&
    !validationData.transaction.quantity[cellValue]
  );
...
function getValidatedObjAttributePathValue(
  ...
  //for Gross Weight UOM
  else if (invalidGrossWeightUOM(csvHeader, csvHeaderCounter, validationData, cellValue)) {
    response.error.push(commonErrorMessage(cellValue, csvHeader, csvHeaderCounter, rowIndex));
  }
  // for Total packaging Unit Price UOM
  else if (invalidTotalPackagingUnitCountUOM(csvHeader, csvHeaderCounter, validationData, cellValue)) {
    response.error.push(commonErrorMessage(cellValue, csvHeader, csvHeaderCounter, rowIndex));
